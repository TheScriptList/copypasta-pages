...
<div class="navbar bg-base-100 shadow-sm border-b border-base-200 sticky top-0 z-50 px-4">
	...
	<div class="flex-none flex items-center gap-2">
		...
		<div class="hidden sm:flex gap-1">
			<a href="{base}/" class="btn btn-ghost btn-sm {page.url.pathname === '/' ? 'btn-active' : ''}">Home</a>
			<a href="{base}/categories" class="btn btn-ghost btn-sm {page.url.pathname === '/categories' ? 'btn-active' : ''}">Categories</a>
			<a href="{base}/settings" class="btn btn-ghost btn-sm {page.url.pathname === '/settings' ? 'btn-active' : ''}">Settings</a>
		</div>
	</div>
</div>
